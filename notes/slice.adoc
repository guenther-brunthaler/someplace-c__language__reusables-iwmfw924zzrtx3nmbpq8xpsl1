...
* "start" - well this sound also good. Maybe even better than "data", because "start" + "length" fit together just fine. However, data is shorter - less to type. "data" + "length" also sound OK, even though "data" + "size" would sound even better.
...
I tried several approaches, and one using unions actually worked. However, it looked like a dirty trick to me, and so I abandoned that approach too.
...
In order to avoid this and use a clean approach, I instead provided the `slice2view_c0()` function which copies the two common fields from the writable `slice` to the read-only `slice_view`.
